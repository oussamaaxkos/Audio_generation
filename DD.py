...
def separate_audio(input_path, output_dir, job_id, model_name='htdemucs'):
    try:
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        thread = Thread(target=separate_audio, args=(input_path, job_output_dir, job_id))
        thread.daemon = True
        thread.start()
...
def start_voice_assistant():
    # Your logic here...
    threading.Thread(target=voice_assistant.run()).start()
